...
export function postMapper(entry: Entry<RawPost>): Post {
    ...
    return {...entry.fields, slug, category: category.fields.name.toLowerCase(), excerpt, assets};
}
...
function getAssets(entry: Entry<RawPost>):Asset[] {
    const assetBlocks = entry.fields.content.content.filter(({nodeType})=>nodeType==='embedded-asset-block');
